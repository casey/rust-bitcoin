...
pub struct FundRawTransactionOptions {
    ...
    /// The fee rate in BTC/kvB. NB. This field is converted to camelCase when
    /// serialized, so it is receeived by fundrawtransaction as `feeRate`,
    /// which is BTC/kvB, and *not* `fee_rate`, which is sat/vB.
    #[serde(
        with = "bitcoin::amount::serde::as_btc::opt",
        skip_serializing_if = "Option::is_none"
    )]
